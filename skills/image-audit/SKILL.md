...
const records=[],data_urls=[];let compTotal=0;
for(let i=0;i<imgs.length;i++){const f=imgs[i],fp=path.join(PIC_DIR,f),osz=fs.statSync(fp).size;
try{const buf=await sharp(fp).resize({width:500,height:500,fit:'inside',withoutEnlargement:true}).limitInputPixels(false).jpeg({quality:40}).toBuffer();
